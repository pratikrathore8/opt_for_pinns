...
"""
Helper function for projecting trajectories onto their first two principal components. 
...
def project_trajectories(weights_hist, final_weights, device='cuda'):
	...
	for weights in weights_hist:
		...
		flattened_diff = torch.cat([w.view(w.numel()) if w.dim() > 1 else torch.Tensor(
			w).type(type(w)).cuda(device=device) for w in diff])
...
def generate_loss_grid(directions, final_weights, data, pred_func, loss_func, model, device='cuda', num_samples_x=100, num_samples_y=100, sample_range_x=[-1, 1], sample_range_y=[-1, 1], log_scale=True):
	...
	# generate grid
	xcoords = np.linspace(
		sample_range_x[0], sample_range_x[1], num=num_samples_x+1)
	ycoords = np.linspace(
		sample_range_y[0], sample_range_y[1], num=num_samples_y+1)
	...
	for coord_change in coordinates:
		...
		for (p, w) in zip(model.parameters(), final_weights):
			...
			p.data = w + torch.Tensor(step).type(type(w)
												).view(w.size()).cuda(device=device)
			offset += numel
...
def plot_trajectories(loss_grid, coord_grid, trajectory_coords, variance_ratios, countour_range=None, num_contours=100, save_file="trajectories", animate=False, animation_format='gif'):
	...
	contour_plot = plt.contour(
		coord_grid[0], coord_grid[1], loss_grid, cmap='summer', levels=countour_levels)
	plt.clabel(contour_plot, inline=1, fontsize=8)
	# plot trajectories
	plt.plot(trajectory_coords[0], trajectory_coords[1],
			marker='.', color='firebrick', markersize=10)
	...
	if animate:
		...
		contour_plot = plt.contour(
			coord_grid[0], coord_grid[1], loss_grid, cmap='summer', levels=countour_levels)
		plt.plot(trajectory_coords[0], trajectory_coords[1],
					marker='None', color='firebrick', alpha=0.35, linewidth=1)
		plt.xlabel('1st PC: %.2f %%' % (variance_ratios[0]*100), fontsize='large')
		plt.ylabel('2nd PC: %.2f %%' % (variance_ratios[1]*100), fontsize='large')
		...
		animation = FuncAnimation(
			fig, animate, frames=num_trajectories, interval=50)
		if save_file is not None:
			animation.save(save_file + "." + animation_format,
						dpi=120, writer="pillow")
...
def compute_hessian_directions(data, pred_func, loss_func, model, device='cuda'):
	...
	hessian_comp = hessian(model, pred_func, loss_func,
							data, cuda=use_cuda, device=device)
	_, eig_vecs = hessian_comp.eigenvalues(maxIter=100, tol=1e-3, top_n=2)

	# flatten the eigenvectors
	directions = []
	directions.append(torch.cat([w.view(w.numel()) if w.dim() > 1 else torch.Tensor(
		w).type(type(w)).cuda(device=device) for w in eig_vecs[0]]).cpu().numpy())
	directions.append(torch.cat([w.view(w.numel()) if w.dim() > 1 else torch.Tensor(
		w).type(type(w)).cuda(device=device) for w in eig_vecs[1]]).cpu().numpy())
...
def plot_landscape(loss_grid, coord_grid, countour_range=None, num_contours=100, save_file="landscape"):
	...
	if countour_range is None:
		countour_range = [np.floor(np.min(loss_grid)), np.ceil(np.max(loss_grid))]
	countour_levels = np.linspace(
		countour_range[0], countour_range[1], num_contours)
